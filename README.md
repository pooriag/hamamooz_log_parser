...
- Use `--path` for the raw log file and `--hllpath` for optional global HyperLogLog storage path.
...
## hyperloglog
it has nothing to do the parsing itself and the usecase is expalined in problem section. it is used for unique ip cardinality both globally and per hour.
...
### solution:
since we only want to keep track of count of unique ids a very resource efficient way is using hyperloglog.
...
BUT a more EFICIENT way is to use hyperloglog which is another storage system that uses hashing. it takes much smaller space and its more cost efficient when we need to merge multiple files of this storage system
hyperloglog is designed to keep track of cardinality.
